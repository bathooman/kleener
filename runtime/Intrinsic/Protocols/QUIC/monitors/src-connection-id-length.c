...
* in bytes of the Source Connection ID field that follows it. This length
* is encoded as an 8-bit unsigned integer. In QUIC version 1, this value MUST NOT
* exceed 20 bytes. Endpoints that receive a version 1 long header with a value 
...
{    
    if (is_packet_client_generated && local_state == INITIAL && is_header_long(&pkt->base) && byte_to_int(pkt->version, 4) == Version1)
    {
        ...
        sprintf(symbolic_name, "%s:%"PRIu64, "source_id_length_PN", packet_number);
