#include "klee/Protocols/dtls/dtls_monitors.h"
#include "klee/Protocols/dtls/dtls_records.h"
#include "klee/klee.h"
#include <assert.h>

#define INITIAL 0
#define RECORD_RECEIVED 1
#define EXIT 2

static STATE local_state = INITIAL;

void ch0_version_diff_testing_server(RECORD *P,
                                     bool is_record_client_generated) {
  if (is_record_client_generated && local_state == INITIAL) {
    if (P->content_type == Handshake_REC &&
        P->RES.fragment->handshake_type == Client_Hello_MSG &&
        P->RES.fragment->body.client_hello->cookie_length == 0) {
      uint8_t handshake_version;
      klee_make_symbolic(&handshake_version, 1, "ch0_handshake_version");
      P->RES.fragment->body.client_hello->handshake_version[1] =
          handshake_version;
      local_state = RECORD_RECEIVED;
    }

  } else if (!is_record_client_generated && local_state == RECORD_RECEIVED) {
    generate_dtls_output(P);
    local_state = EXIT;
  } else if (local_state == EXIT) {
    return;
  }
}
