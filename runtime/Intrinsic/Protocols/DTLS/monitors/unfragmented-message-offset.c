...
void is_unfragmented_message_offset_valid_server(RECORD *record, bool is_record_client_generated)
{
    if (is_record_client_generated && local_state == INITIAL)
    {
        ...
        */ 
        ...
        {
            // Fragment offset is made symbolic and assumed to be invalid
            kleener_make_symbolic(record->RES.fragment->fragment_offset, FRAGMENT_OFFSET_LENGTH, "fragment_offset");
            klee_assume(byte_to_int(record->RES.fragment->fragment_offset, FRAGMENT_OFFSET_LENGTH) != VALID_OFFSET);
